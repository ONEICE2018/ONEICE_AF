...
{
    if (watched == ui->widget_cuvers)       //首先判断控件(这里指 lineEdit1)
    {
        if (event->type() == QEvent::Enter)   //然后再判断控件的具体事件 (这里指获得焦点事件)
        {
            ui->widget_cuvers->setFocus();
        }
    }
...
void curve_fitting::CF_Datainit_fixd4LineClass()
{
    CF_LC_Load(CF_init_LC_BasePath, CF_init_LC_filename);
    //将前4行重新写入固定信息（为了固定前四个信息）
    QMap<QString, int> tempclassid;
    tempclassid.insert("FREQ,DAC,ADC,POWER", 0);
    tempclassid.insert("FREQ,DAC,ADC,POWERF", 1);
    tempclassid.insert("FREQ,DAC,POWER,DAC", 2);
    tempclassid.insert("FREQ,DAC,POWER,POWER", 3);
    if(CF_init_ClassID.size() > 4)
    {
        QMapIterator<QString, int> i0(CF_init_ClassID);
        while (i0.hasNext())
        {
            i0.next();
            if(i0.value() > 3)
            {
                tempclassid.insert(i0.key(), i0.value());
            }
        }
    }
    //先清空文件
    CF_LC_Clear(CF_init_LC_BasePath, CF_init_LC_filename);
    //遍历写入
    QMapIterator<QString, int> i1(tempclassid);
    while (i1.hasNext())
    {
        i1.next();
        QString SaveLine = QString("%1,%2").arg(i1.value()).arg(i1.key());
        CF_LC_Write_ONE(CF_init_LC_BasePath, CF_init_LC_filename, SaveLine);
    }
}
void curve_fitting::CF_init()
{
    CF_Datainit_fixd4LineClass();//固定前四个种类
    ...
    QObject::connect(ui->list_projects, SIGNAL(OneListWidget_ItemClicked(int)), this, SLOT(on_OneListWidget_Project_ItemClicked(int)));
    ...
    QObject::connect(ui->list_lineclass, SIGNAL(OneListWidget_ItemClicked(int)), this, SLOT(on_OneListWidget_LineClass_ItemClicked(int)));
    ...
    QObject::connect(ui->list_lineindex, SIGNAL(OneListWidget_ItemClicked(int)), this, SLOT(on_OneListWidget_Index_ItemClicked(int)));
    //连接曲线界面 监听数据变化
    ...
    ui->widget_cuvers->installEventFilter(this);
    //初始话工作空间路径
...
int curve_fitting::CF_LoadProject(QString proname)//不需要带.bin的名字
{
    //3加载文件内容
    QFile f(QString(WorkSpace + "\\" + proname + ".bin").replace("/", "\\"));
...
int curve_fitting::CF_Load(QString ws)
{
    ...
    {
        if(proname.split(".").length() == 2 && proname.split(".")[1] == "bin")
        {
...
void curve_fitting:: nowCFProject_insert(QString proname,  QMap<uint16_t, QMap<uint16_t, FinalDatas>> data)
{
    ...
    {
        AF_LineFile.setFileName(QString(WorkSpace + "\\" + nowCFProject + ".bin").replace("/", "\\"));
...
void curve_fitting::on_OneListWidget_Project_ItemClicked(int row)
{
    on_list_projects_currentRowChanged(row);
}
...
void curve_fitting::on_OneListWidget_LineClass_ItemClicked(int row)
{
    on_list_lineclass_currentRowChanged(row);
}
...
void curve_fitting::on_OneListWidget_Index_ItemClicked(int row)
{
    on_list_lineindex_currentRowChanged(row);
}
...
void curve_fitting::on_list_lineindex_currentRowChanged(int currentRow)
{
            ...
            && ui->list_lineindex->item(currentRow)->text() != "")
    {
        ...
        //if(nowIndex != choiced)
        {
            ...
            show_LineDatas(nowCFProject, nowlineClass.toInt(), nowIndex.toInt());
            if(nowCFProject == "WSPS_7205_16KW")
            {
                MainWindow::nowMainWindow->pm_setfreq((71 + (nowIndex.toDouble() / 10000.0)), MainWindow::nowMainWindow->returnchmsg(MainWindow::nowchanal));
            }
            else
            {
...
{
    ui->progressBar_AFUpload->setValue(0);
    AF_updataFile.setFileName(QString(WorkSpace + "\\" + "LoadData" + ".bin").replace("/", "\\"));
    if(AF_updataFile.isOpen())
    {
        AF_updataFile.close();
    }
...
void curve_fitting::on_AF_STLineDownLoad_clicked()
{
    ...
    if(nowCFProject != NULL)
    {
        ...
        {
            case QMessageBox::Ok:        //主代码烧写
                result_choice = 1;
                break;
            case QMessageBox::YesToAll :    //子代码烧写 不用了
                result_choice = 2;
                break;
    ...
    {
        f = QString(WorkSpace + "\\" + nowCFProject + ".bin").replace("/", "\\");
        cpuused_fname = nowCFProject;
    }
    else
    {
